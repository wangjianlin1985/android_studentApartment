...
public class ClassInfoAction extends BaseAction {
    ...
    public String QueryClassInfo() {
        ...
        if(banzhuren == null) banzhuren = "";
        if(beginDate == null) beginDate = "";
        List<ClassInfo> classInfoList = classInfoDAO.QueryClassInfoInfo(classNo, className, banzhuren, beginDate, currentPage);
        /*计算总的页数和总的记录数*/
        classInfoDAO.CalculateTotalPageAndRecordNumber(classNo, className, banzhuren, beginDate);
        /*获取到总的页码数目*/
    ...
    public String QueryClassInfoOutputToExcel() { 
        ...
        if(banzhuren == null) banzhuren = "";
        if(beginDate == null) beginDate = "";
        List<ClassInfo> classInfoList = classInfoDAO.QueryClassInfoInfo(classNo,className,banzhuren,beginDate);
        ExportExcelUtil ex = new ExportExcelUtil();
        String title = "ClassInfo信息记录"; 
        String[] headers = { "班级编号","班级名称","班主任姓名","成立日期"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<classInfoList.size();i++) {
        	ClassInfo classInfo = classInfoList.get(i); 
        	dataset.add(new String[]{classInfo.getClassNo(),classInfo.getClassName(),classInfo.getBanzhuren(),new SimpleDateFormat("yyyy-MM-dd").format(classInfo.getBeginDate())});
        }
        /*
