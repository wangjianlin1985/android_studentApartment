...
public class BuildingInfoDAO {
    ...
    public ArrayList<BuildingInfo> QueryBuildingInfoInfo(String buildingName,int currentPage) { 
    	Session s = factory.getCurrentSession();
    ...
    @Transactional(propagation=Propagation.NOT_SUPPORTED)
    public ArrayList<BuildingInfo> QueryBuildingInfoInfo(String buildingName) { 
    	Session s = factory.getCurrentSession();
    	String hql = "From BuildingInfo buildingInfo where 1=1";
    	if(!buildingName.equals("")) hql = hql + " and buildingInfo.buildingName like '%" + buildingName + "%'";
    	Query q = s.createQuery(hql);
    ...
    /*计算总的页数和记录数*/
    @Transactional(propagation=Propagation.NOT_SUPPORTED)
    public void CalculateTotalPageAndRecordNumber(String buildingName) {
        Session s = factory.getCurrentSession();
        String hql = "From BuildingInfo buildingInfo where 1=1";
        if(!buildingName.equals("")) hql = hql + " and buildingInfo.buildingName like '%" + buildingName + "%'";
        Query q = s.createQuery(hql);
